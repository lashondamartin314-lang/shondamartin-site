...
const phases = [
  ...
  },
  {
    ...
    outcome: 'Members organize evidence, identify violations, and escalate only when the dispute record supports it.',
    templates: []
  }
];
...
const templates = [
  ...
  { code: 'L18', phase: 'phase-5', title: 'Pay for Delete Offer', use: 'Use only when negotiation makes sense and the member can get any agreement in writing first.', timing: 'Negotiation option.', link: 'https://docs.google.com/document/d/10OE3CfVa_XCFKCP85wHwB1q9VxxaSo1uXWDo5mVb4iA/copy', viewLink: 'https://docs.google.com/document/d/10OE3CfVa_XCFKCP85wHwB1q9VxxaSo1uXWDo5mVb4iA/edit' },
...
const resources = [
  { title: 'Full Dispute Playbook Manual', desc: 'Long-form reference version of the playbook.', url: '../../repairplaybook/kit.html', type: 'Manual' },
  { title: 'Printable Playbook PDF', desc: 'Current printable PDF while the Dispute Playbook rename is being updated.', url: '../../repairplaybook/The%20Repair%20Playbook%20%C2%B7%20Credit%20Academy%20%C2%B7%20Shonda%20Martin.pdf', type: 'PDF' },
  ...
  { title: 'Credit Academy Community', desc: 'Ask questions, find updates, and get help from the member community.', url: COMMUNITY_URL, type: 'Community' }
];
...
const faqs = [
  ...
  { q: 'Can members type personal account details into the tracker?', a: 'The tracker saves in the member’s browser only, but members should still avoid full SSNs, full account numbers, and sensitive documents. Use short labels and last four digits.' },
  { q: 'Why use certified mail?', a: 'Certified mail gives the member a date-stamped paper trail. That paper trail matters if a bureau, collector, or furnisher fails to respond or later claims it did not receive the dispute.' },
  { q: 'Are these templates legal advice?', a: 'No. The hub is educational and self-help oriented. Members can use the information to understand their rights and prepare their own disputes, and they should contact an attorney for legal advice.' },
  { q: 'Where do I ask for help?', a: 'Use Community Help for general questions, FAQs, and Circle support. For private legal questions, members should speak with a consumer rights attorney.' }
];

const initialParams = new URLSearchParams(window.location.search);
const initialView = ['start', 'path', 'templates', 'tracker', 'decoder', 'resources', 'help'].includes(initialParams.get('view'))
...
function renderStart() {
  ...
  view.innerHTML = `
    ${sectionHead('Start Here', 'One hub for the whole dispute process', 'Members can begin with the checklist, follow the Dispute Path, open letter templates, track outgoing mail, decode responses, and find help without bouncing between disconnected pages.')}
    <div class="pb-grid">
      ...
      <article class="pb-card">
        ...
        <p>Use the canonical ${templateCount} letter templates. Drive links can be added as soon as the source docs are ready.</p>
    ...
    <div class="pb-layout">
      <section class="pb-panel">
        <h3>Member Readiness Checklist</h3>
      ...
      <aside class="pb-panel">
        <h3>Privacy Note</h3>
        <p>This tracker stores data in this browser only. Members should use short names, last four digits, or nicknames instead of full account numbers, SSNs, or private document details.</p>
        <div class="pb-disclaimer">Educational self-help tool only. It helps members organize their own dispute process; it does not replace legal advice.</div>
      </aside>
    </div>
  `;
}
...
function renderPath() {
  view.innerHTML = `
    ${sectionHead('Dispute Path', 'A clear phase-by-phase workflow', 'The hub now points members through one canonical path. Each phase explains the goal, the expected outcome, and the templates attached to that point in the process.')}
        ...
        <article class="pb-phase-card ${phase.id}">
          ...
          <div>
            ...
            <div class="pb-phase-meta">
              <span class="pb-phase-chip">${phase.templates.length} templates</span>
              <span class="pb-phase-chip">${escapeHtml(phase.outcome)}</span>
            </div>
          </div>
          <button class="pb-btn pb-btn-plain" data-phase="${phase.id}">See Templates</button>
        </article>
      `).join('')}
    </div>
  `;
...
function renderTemplates() {
  ...
  view.innerHTML = `
    ${sectionHead('Letter Templates', 'A growing template library', 'Each card explains when to use the template and where it belongs in the Dispute Path. New templates can be added as the member resource library grows.')}
    <div class="pb-filter-row">
      ...
      <select class="pb-select" id="phase-filter" aria-label="Filter by phase">
        ...
        ${phases.map(phase => `<option value="${phase.id}" ${state.phaseFilter === phase.id ? 'selected' : ''}>Phase ${phase.number} - ${escapeHtml(phase.title)}</option>`).join('')}
      </select>
    </div>
...
function renderTracker() {
  view.innerHTML = `
    ${sectionHead('Tracker', 'Track what was sent and what came back', 'This local tracker helps members document their paper trail by account, bureau, template, certified mail number, response, and next action.')}
    <div class="pb-layout">
      <section class="pb-panel">
...
function renderDecoder() {
  ...
  view.innerHTML = `
    ${sectionHead('Response Decoder', 'Turn bureau language into a next step', 'Members can match the response they received and see what it usually means, what to save, and which template may be next.')}
...
function renderResources() {
  view.innerHTML = `
    ${sectionHead('Resources', 'Everything members need close by', 'This area gathers the full manual, template vault, official complaint links, credit report access, and community resources.')}
...
function renderHelp() {
  view.innerHTML = `
    ${sectionHead('Community Help', 'FAQs, boundaries, and support paths', 'Members should know where to ask general questions, where to find updates, and when a question needs professional legal guidance.')}
